...
(df_or
 ...
 .size()
 .plot
 .bar(title="Volume des transactions par heure", rot=0, ax=ax)
);
...
(df_or.UK.value_counts()
 ...
 .plot
 .pie(title="Volume des transactions UK / Non UK", autopct='%.1f%%', ax=ax)
 .set_ylabel("")
);
...
(df_or
 ...
 .sort_index(ascending=False)
 .plot
 .pie(title="Montant des transactions UK / Non UK", autopct='%.1f%%', ax=ax)
 .set_ylabel("")
);
...
(df_or.Country.value_counts()
 .drop("United Kingdom")
 .plot
 .barh(title="Volume des transactions par pays Non UK", ax=ax)
)
...
(df_or
 ...
 .plot
 .barh(title="Montant des transactions par pays Non UK", ax=ax)
)
...
(df_or
 ...
 .unstack()
 .plot
 .bar(title="Volume des transactions par type", color=['g', 'r'], rot=0, ax=ax)
)
...
(df_or
 ...
 .unstack()
 .plot
 .bar(title="Montant des transactions par type", color=['g', 'r'], rot=0, ax=ax)
)
